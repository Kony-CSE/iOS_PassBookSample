...
function ensurePathFP(path) {
    ...
    file = myfile;
    if (!myfile.exists()) {		
        if (typeof pth == "string" && pth.indexOf(".") > 0) 
        	myfile.createFile();
    	...
    	request.onReadyStateChange = function() {
            if (request.readyState == constants.HTTP_READY_STATE_DONE) {
				...
				for(var k=0;k<passes.length;k++){
					var passSet = [];
					if(passes[k].indexOf("pkpass", passes[k].length - 6) !== -1)
						passSet.push(passes[k].substring(0, passes[k].length-7));
					else
				...
				if(passes.length>0){
					var chkBasic = { id: "checkBox", isVisible: true, masterData:masterPasses, skin:"chkSkin", focusSkin:"chkFocus", onSelection:onSelCallBck};
...
function addPassesToPassbook(){
	...
	if(passesSelected.length==1){
		...
		addPassWithPath(kony.io.FileSystem.getDataDirectoryPath() + constants.FILE_PATH_SEPARATOR + passesSelected[0][1],true,function(){alert("Pass added from URL");});
	}
    	...
    	addPassesWithPath(passesFromDisk, true, function(){alert("Two Passes added to the Passbook")});
	}
}
